document.addEventListener('DOMContentLoaded', () => {
    ...
    // MUZYKA W TLE (jeśli masz element <audio id="bg-music"> w HTML)
    const music = document.getElementById('bg-music');

    // --- SYNTEZATOR EFEKTÓW DŹWIĘKOWYCH (Generowane w locie) ---
    let audioCtx;
    function initAudio() {
        if (!audioCtx) {
        ...
        if (audioCtx.state === 'suspended') {
            audioCtx.resume(); // Wybudza dźwięk zablokowany przez przeglądarkę
        }
    }
    ...
    // Wywołania konkretnych dźwięków
    const sndMiss = () => playNoise(0.3, 'highpass', 1000, 0.5); // Dźwięk fali
    const sndHit = () => { playTone(120, 'square', 0.4, 0.5); playNoise(0.4, 'lowpass', 400, 0.7); }; // Armata
    const sndSink = () => { playTone(60, 'sawtooth', 0.8, 0.8); playNoise(1.0, 'lowpass', 200, 1.0); }; // Zatopienie

    function sndWin() {
        if (!audioCtx) return;
        const t = audioCtx.currentTime;
        [440, 554, 659, 880].forEach((f, i) => playTone(f, 'sine', 0.5, 0.4, t + i * 0.15));
    }

    function sndLose() {
        if (!audioCtx) return;
        const t = audioCtx.currentTime;
        [300, 280, 260, 220].forEach((f, i) => playTone(f, 'sawtooth', 0.6, 0.4, t + i * 0.3));
    }
    // -----------------------------------------------------------
    ...
    playBtn.addEventListener('click', () => {
        initAudio(); // Budzimy efekty dźwiękowe
        document.getElementById('main-menu').classList.add('hidden');
        document.getElementById('game-ui').classList.remove('hidden');
        
        // Puszczamy muzykę w tle z Twojego pliku HTML
        if(music) {
            music.volume = 0.1;
            music.play().catch(() => console.log("Przeglądarka zablokowała muzykę z pliku."));
        }
        initGame();
    });
    ...
    function playerAttack(id, cell) {
        ...
        if (ship) {
            ...
            if (++ship.hits === ship.len) {
                ...
                sndHit(); // Syntezator - trafienie
            }
            if (cpuHealth <= 0) endGame(true);
        } else {
            cell.classList.add('miss'); 
            sndMiss(); // Syntezator - pudło
    ...
    // [NIE RUSZAŁEM LOGIKI AI Z TWOJEGO KODU]
    ...
    function endGame(isWin) {
        ...
        if(music) music.pause(); // Wycisz muzykę na koniec
        isWin ? sndWin() : sndLose(); // Odegraj fanfary z syntezatora

        // Pokazanie statków wroga
